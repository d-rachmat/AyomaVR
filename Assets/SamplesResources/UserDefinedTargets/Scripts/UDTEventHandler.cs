    ...
    public void BuildNewTarget()
    {
			...
			mFrameQuality == ImageTargetBuilder.FrameQuality.FRAME_QUALITY_HIGH)
        {
			...
			for (int i = 0; i < vidio.Length; i++) 
			{
				if (vidio[i]!=null) 
				{
					VideoPlaybackBehaviour video = vidio[i].GetComponent<VideoPlaybackBehaviour>();
					video.VideoPlayer.Play(false, 0);
				}
			}
			GameObject[] mIco = GameObject.FindGameObjectsWithTag("ICON");
			for (int i = 0; i < mIco.Length; i++) 
			{
				mIco [i].SetActive (false);
			}

			//-----------------------------------------------------------
			ImageTargetTemplate.BuildPnl.SetActive(false);

			for (int i = 0; i < ImageTargetTemplate.allGO.Length; i++) 
			{
				//ImageTargetTemplate.allGO [i].SetActive (false);
			}

			ImageTargetTemplate.UIChoice.SetActive (true);

			GameObject evs = GameObject.Find ("EventSystem");
			if (evs!=null) 
			{
				evs.GetComponent<wlk> ().ok.SetActive (true);
			}
        ...
        else
        {
			...
			if (evs!=null) 
			{
				evs.GetComponent<wlk> ().ok.SetActive (false);
			}
